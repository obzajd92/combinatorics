...
# --- ROLLING LOOKBACK & SIZING SETTINGS ---
USER_INITIAL_SIZING_INPUT = 0.04  # 4% initial bankroll allocation fraction
LOOKBACK_WINDOW = 20             # Strict rolling lookback window size for adaptive win-rate metric
...
def simulate_walk(start_balance, steps, step_func, initial_sizing, lookback):
    ...
    # Track historical returns to compute rolling metrics
    recent_returns = []
    
    for _ in range(steps):
        # --- ADAPTIVE ROLLING KELLY ENGINE ---
        if len(recent_returns) >= 5:
            # Slice only the most recent N steps based on the lookback window
            window_slice = recent_returns[-lookback:]
            win_rate = sum(1 for r in window_slice if r > 0) / len(window_slice)
            
            # Simple Kelly variant: f = win_rate - loss_rate
...
def calculate_ruin_probability(initial_balance, steps, step_func, initial_sizing, lookback, num_sims=NUM_SIMULATIONS):
    """Calculates empirical probability of triggering the dynamic floor under specific sizing constraints."""
    ruined_count = 0
    for _ in range(num_sims):
        walk, _ = simulate_walk(initial_balance, steps, step_func, initial_sizing, lookback)
        if walk[-1] < TAKE_PROFIT and len(walk) <= steps:
            ruined_count += 1
    return ruined_count / num_sims

def run_game_iteration(retry_num, initial_sizing, lookback):
    """Executes a full simulation evaluation cycle with adaptive rolling inputs."""
    print(f"\n--- Running Rolling Lookback Simulation (Run: #{retry_num} | Window: {lookback} steps) ---")
    
    # Generate sample visualization steps
    walks = {}
    floors = {}
    for name, func in STRATEGIES.items():
        walks[name], floors[name] = simulate_walk(START_BALANCE, MAX_STEPS, func, initial_sizing, lookback)
    ...
    for b in balances_to_test:
        for name, func in STRATEGIES.items():
            prob = calculate_ruin_probability(b, MAX_STEPS, func, initial_sizing, lookback)
    ...
    ax1.set_title(f"Random Walks with Rolling Adaptive Kelly (Run #{retry_num})")
    ...
    ax2.set_title(f"Risk Surface Area (Lookback: {lookback})")
...
if __name__ == "__main__":
    ...
    for i in range(1, total_retries + 1):
        round_winner = run_game_iteration(
            retry_num=i, 
            initial_sizing=USER_INITIAL_SIZING_INPUT, 
