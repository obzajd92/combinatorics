...
BASE_LOOKBACK = 20                # Standard tracking lookback
VOLATILITY_THRESHOLD = 250        # Threshold to trigger regime switch
...
def simulate_walk(start_balance, steps, step_func, initial_sizing):
    """Simulates a path with dynamic adaptive Kelly sizing and a regime-switching lookback window."""
    ...
    for _ in range(steps):
        ...
        # Alter lookback windows dynamically based on localized variance spikes
        if len(recent_returns) >= 10:
            rolling_vol = np.std(recent_returns[-10:])
            if rolling_vol > VOLATILITY_THRESHOLD:
                # Volatility Spike (Regime 1) -> Tighten lookback window to react quickly
                active_lookback = 8
            else:
                # Quiet Market (Regime 0) -> Expand lookback window to smooth out noise
                active_lookback = 30
        else:
...
def run_game_iteration(retry_num, initial_sizing):
    ...
    print(f"\n--- Running Regime-Switching Simulation (Run: #{retry_num}) ---")
    ...
    ax1.set_title(f"Random Walks with Regime-Switching Adaptive Kelly (Run #{retry_num})")
    ...
    ax2.set_title("Risk Surface Area Curve (Regime-Adaptive)")
